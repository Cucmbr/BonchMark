...
public class BonchAPI
{
    ...
    internal async Task<string> PullMessagesAsync()
    {
        using LkRequest messagesRequest = new LkRequest("key=840");
        using (var messagesResponse = await _httpClient.PostAsync(_httpClient.BaseAddress + "/cabinet/project/cabinet/forms/message.php", messagesRequest))
        {
            if (InvalidResponseCheck(messagesResponse))
            {
                return "";
            }
            var byteResp = await messagesResponse.Content.ReadAsByteArrayAsync();
            return Encoding.GetEncoding(1251).GetString(byteResp, 0, byteResp.Length);
        }
    }
